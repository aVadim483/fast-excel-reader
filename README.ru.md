...
* Открывается через `Excel::open()` как обычная книга с одним листом и тем же API чтения, что у XLSX/XLS, либо через `Excel::openCsv()` для низкоуровневого движка
...
// И XLSX, и устаревший XLS, и CSV открываются так же — ридер выбирается по сигнатуре файла
...
* [XLS (Excel 97-2003)](docs/ru/21-xls.md) — чтение XLS-файлов
