...
$result['#3'] = $excel
    ...
    ->readRows($columnKeys);
```
Область чтения можно задать по именованным диапазонам в файле. Например, если в файле есть
...
Вызовите `Excel::getProperties()`, чтобы прочитать свойства документа — базовые
...
`company`, `manager` (прикладные). Возвращаются только реально присутствующие в файле свойства; файл
